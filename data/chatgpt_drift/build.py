#!/usr/bin/env python3
"""01_build_response_matrix.py — Download and process ChatGPT/LLM Drift evaluation data.

Downloads from: https://github.com/lchen001/LLMDrift
...
def download():
    ...
    if repo_dir.exists() and (repo_dir / ".git").exists():
        print(f"[SKIP] Repository already cloned at: {repo_dir}")
        ...
        if result.returncode != 0:
            print("[WARN] Could not pull updates (may have diverged). Using existing clone.")
    else:
    ...
    print("\n[INFO] ChatGPT drift data download complete.")
...
def main():
    ...
    if col_model and col_dataset and "_score" in scored.columns:
        ...
        # Build item_content.csv: map each item_id to its query text
        item_content_df = (
            scored.drop_duplicates(subset=["_item_id"])
            .sort_values("_item_id")
            [["_item_id", "query"]]
            .rename(columns={"_item_id": "item_id", "query": "content"})
            .reset_index(drop=True)
        )
        out_path = PROCESSED_DIR / "item_content.csv"
        item_content_df.to_csv(out_path, index=False)
        print(f"\n  Item content: {len(item_content_df)} items")
        print(f"  Saved to: {out_path}")
