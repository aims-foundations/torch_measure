#!/usr/bin/env python3
"""Download and build response matrix for HELM African MMLU + Winogrande (11 languages).
...
Languages: Afrikaans, Amharic, Bambara, Igbo, Sepedi, Shona, Sesotho, Setswana, Tsonga, Xhosa, Zulu
...
PROJECT_PREFIX = "mmlu-winogrande-afr/benchmark_output/runs/"
...
def download(workers: int = 8, latest_only: bool = False) -> None:
    """Download all African MMLU + Winogrande data from GCS."""
    print(f"\n{'=' * 60}")
    print("Downloading: African MMLU + Winogrande (11 languages)")
    print(f"{'=' * 60}")
...
def parse_run_dir_name(run_dir_name: str) -> dict:
    """Parse run directory name to extract task, model, language, subject.
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Download and build response matrix for HELM African MMLU + Winogrande")
    ...
    # Step 2: Process
    print(f"\n{'=' * 60}")
    print("Processing: African MMLU + Winogrande (11 languages)")
    print(f"Data dir: {RAW_DIR}")
    print(f"{'=' * 60}")

    records, metadata = process_project(RAW_DIR, "afr")
