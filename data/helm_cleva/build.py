#!/usr/bin/env python3
"""Download and build response matrix for HELM CLEVA Chinese evaluation.
...
PROJECT_PREFIX = "cleva/benchmark_output/runs/"
...
def download(workers: int = 8, latest_only: bool = False) -> None:
    """Download all CLEVA data from GCS."""
    print(f"\n{'=' * 60}")
    print("Downloading: CLEVA Chinese evaluation (21 tasks)")
    print(f"{'=' * 60}")
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Download and build response matrix for HELM CLEVA")
    ...
    # Step 2: Process
    print(f"\n{'=' * 60}")
    print("Processing: CLEVA Chinese evaluation (21 tasks)")
    print(f"Data dir: {RAW_DIR}")
    print(f"{'=' * 60}")

    records, metadata = process_project(RAW_DIR, "cleva")
