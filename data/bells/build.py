"""
01_build_response_matrix.py — Download and process BELLS benchmark data.

Downloads BELLS (Benchmark for the Evaluation of LLM Supervision) data, which
...
Processes BELLS sub-datasets (hallucination, jailbreak, prompt injection,
machiavelli, tensor_trust, etc.). Loads JSONL files from raw/bells_datasets/
...
def download():
    """Download BELLS benchmark data: clone repo and download JSONL files."""
    RAW_DIR.mkdir(parents=True, exist_ok=True)

    # Clone the repository
    repo_dir = RAW_DIR / "BELLS"
    if repo_dir.exists():
        print("BELLS repository already exists, skipping clone.")
    else:
        print("Cloning BELLS repository...")
        subprocess.run(
            ["git", "clone", "--depth", "1",
             "https://github.com/CentreSecuriteIA/BELLS.git", str(repo_dir)],
            check=True,
        )
        print("Done cloning repository.")

    # Download JSONL dataset files from the BELLS website
    datasets_dir = RAW_DIR / "bells_datasets"
    datasets_dir.mkdir(parents=True, exist_ok=True)

    base_url = "https://bells.therandom.space/datasets"
