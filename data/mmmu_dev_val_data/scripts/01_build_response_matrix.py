"""
Build MMMU_DEV_VAL response matrix from VLMEval/OpenVLMRecords.

Data source:
  - HuggingFace dataset: VLMEval/OpenVLMRecords
  - Per-model xlsx files at mmeval/{model}/{model}_MMMU_DEV_VAL.xlsx
  - College-level multimodal MCQ benchmark (validation split)
...
Outputs:
  - raw/{model}_MMMU_DEV_VAL.xlsx: Downloaded xlsx files
...
def main():
    print(f"MMMU_DEV_VAL Response Matrix Builder")
    print("=" * 60)
    ...
    for i, model in enumerate(models_with_bench):
        ...
        try:
            ...
            if item_info is None:
                ...
                for c in ["category", "l2-category", "answer", "question_type", "A", "B", "C", "D"]:
                    if c in df.columns:
                        item_cols.append(c)
