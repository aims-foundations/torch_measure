"""
01_build_response_matrix.py — Download and process Sycophancy to Subterfuge dataset.

Downloads Anthropic's sycophancy-to-subterfuge-paper repository
(https://github.com/anthropics/sycophancy-to-subterfuge-paper), which studies how
sycophantic behavior in LLMs can escalate into more harmful subterfuge behaviors,
then processes the stage-level JSONL.GZ sample data showing the escalation from
political sycophancy through tool use flattery, rubric modification, to reward tampering.
...
def download():
    ...
    if repo_dir.exists():
        print("sycophancy-to-subterfuge-paper already cloned, skipping download")
        return
    print("Cloning sycophancy-to-subterfuge-paper repo...")
    subprocess.run(
        ["git", "clone", "--depth", "1", "https://github.com/anthropics/sycophancy-to-subterfuge-paper.git", str(repo_dir)],
        check=True,
    )
