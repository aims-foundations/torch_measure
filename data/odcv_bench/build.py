"""
01_build_response_matrix.py — Download and process ODCV-Bench dataset.

Downloads the ODCV-Bench repository (Out-of-Distribution Computer Vision benchmark evaluating
model robustness to distribution shifts in vision tasks) from
https://github.com/McGill-DMaS/ODCV-Bench, then builds model x scenario misalignment matrix
from existing_results/ score CSVs and scenario definitions.
...
def download():
    ...
    if repo_dir.exists():
        print("ODCV-Bench already cloned, skipping download")
        return
    print("Cloning ODCV-Bench repo...")
    subprocess.run(
        ["git", "clone", "--depth", "1", "https://github.com/McGill-DMaS/ODCV-Bench.git", str(repo_dir)],
        check=True,
    )
