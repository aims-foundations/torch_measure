...
def download():
    ...
    repo_dir = RAW_DIR
    if repo_dir.exists():
        print("repo already cloned, skipping download")
        return
    print("Cloning BBQ repo...")
    subprocess.run(
        ["git", "clone", "--depth", "1", "https://github.com/nyu-mll/BBQ.git", str(repo_dir)],
        check=True,
    )
...
def main():
    ...
    # Build item_content.csv: map each item_id to its question text
    item_content_rows = []
    for cat in CATEGORIES:
        data_path = RAW_DIR / "data" / f"{cat}.jsonl"
        if not data_path.exists():
