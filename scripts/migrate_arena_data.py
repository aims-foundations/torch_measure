...
"""Migrate Chatbot Arena pairwise comparisons from stair-lab to torch-measure-data.

Downloads pairwise comparison data from stair-lab/chatbot_arena, converts it
to the torch_measure PairwiseComparisons .pt format, and uploads to HuggingFace.

Usage:
    export HF_TOKEN=hf_xxxxx
    python scripts/migrate_arena_data.py

Source data format (parquet, 23.3k rows):
    model_a, model_b: str — model names
    winner: str — "model_a", "model_b", "tie", "tie (bothbad)"
    question_id: str — unique comparison identifier
    conversation_a / conversation_b: list[dict] — full conversations
    language, judge, turn, anony: comparison metadata
...
def load_arena_data() -> pd.DataFrame:
    """Download the chatbot arena parquet from HuggingFace."""
    print("Downloading chatbot arena data...")
    path = hf_hub_download(SRC_REPO, "data/train-00000-of-00001.parquet", repo_type="dataset")
...
def build_subject_metadata(subject_ids: list[str]) -> list[dict]:
    ...
    for sid in subject_ids:
        # Arena model names don't follow org/model convention consistently,
        # but we can try to extract common patterns.
...
def process_arena_data(df: pd.DataFrame) -> dict:
    """Convert arena DataFrame into the .pt payload format."""
    print("\nProcessing arena data...")
    ...
    # Items (prompts) — deduplicate by question_id
    raw_question_ids = df["question_id"].tolist()
    ...
    for qid, text in zip(raw_question_ids, raw_prompt_texts):
    ...
    for _, row in df.iterrows():
        comparison_metadata.append({
            ...
            "anony": bool(row.get("anony", False)),
        })
