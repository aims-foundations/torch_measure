...
class TestMLEFit:
    def test_reduces_loss(self, small_response_matrix):
        ...
        history = mle_fit(model, small_response_matrix, mask, max_epochs=100, verbose=False)
    ...
    def test_lbfgs_optimizer(self, small_response_matrix):
        model = Rasch(n_subjects=20, n_items=30)
        mask = torch.ones_like(small_response_matrix, dtype=torch.bool)
        history = mle_fit(model, small_response_matrix, mask, max_epochs=50, verbose=False, optimizer_cls="lbfgs")
        assert len(history["losses"]) > 0

    def test_convergence_stops_early(self, small_response_matrix):
        model = Rasch(n_subjects=20, n_items=30)
        mask = torch.ones_like(small_response_matrix, dtype=torch.bool)
        history = mle_fit(model, small_response_matrix, mask, max_epochs=10000, convergence_tol=1e-4, verbose=False)
    ...
    def test_with_mask(self, small_response_matrix):
        ...
        mask = torch.ones_like(small_response_matrix, dtype=torch.bool)
        mask[:5, :5] = False
        history = mle_fit(model, small_response_matrix, mask, max_epochs=50, verbose=False)
        assert len(history["losses"]) > 0
